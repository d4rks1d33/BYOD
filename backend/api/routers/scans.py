...
@router.post("/projects/{project_id}/upload-postman", response_model=dict)
...
@router.get("/projects/{project_id}/scans/latest/report/markdown", response_class=Response)
async def get_latest_report_markdown(
    ...
    if not scan:
        raise HTTPException(status_code=404, detail={"code": "NO_COMPLETED_SCAN", "message": "No completed scan found for this project"})

    report_content = scan.statistics.get("report", "")
    if not report_content:
        raise HTTPException(status_code=404, detail={"code": "REPORT_NOT_FOUND", "message": "Report content not found in scan statistics"})

    return Response(
        content=report_content,
        media_type="text/markdown",
        headers={"Content-Disposition": f"attachment; filename=report-{scan.id}.md"}
    )
