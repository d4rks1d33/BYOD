...
- Test for application-specific logic flaws
...
Be thorough but efficient. Focus on HIGH-IMPACT vulnerabilities first."""
    ...
    def _parse_findings(self, content: str, context: AgentContext):
        ...
        # Match FINDING: [severity] title - description
        pattern = r'FINDING:\s*\[([^\]]+)\]\s*(.+?)\s*-\s*(.+?)(?=\n|$)'
        ...
        for match in matches:
            ...
            if not any(f["title"] == title for f in context.raw_findings):
                ...
                # Also log to scan logger with finding-specific formatting
                try:
                    from services.scan_logger import ScanLogger
                    sl = ScanLogger.get_current()
                    if sl:
                        sl.finding(self.name, severity, title, description)
                except Exception:
                    pass
...
class AnalysisAgent(BaseAgent):
    ...
    def analyze(self, context: AgentContext) -> Dict[str, Any]:
        ...
        self.log("INFO", f"Analyzing {len(context.raw_findings)} raw findings")

        if not context.raw_findings:
            self.log("INFO", "No findings to analyze")
            return {"validated_count": 0}

        findings_text = json.dumps(context.raw_findings, indent=2)[:5000]

        task = f"""Analyze and validate these security findings:

{findings_text}

For each finding:
1. Validate if it's a REAL vulnerability or false positive
2. Score severity using CVSS 3.1
3. Assign CWE ID
4. Map to OWASP category
5. Deduplicate similar findings
6. Provide concrete remediation

Output a JSON array of VALIDATED_FINDINGS."""

        result = self.run(task, context, max_iterations=10)

        # Extract validated findings from output
        for msg in result["messages"]:
            if msg["role"] == "assistant":
                content = msg.get("content", "")
                if "VALIDATED_FINDINGS" in content:
                    try:
                        # Find the JSON array
                        start = content.index("[", content.index("VALIDATED_FINDINGS"))
                        json_str = self._extract_json_array(content[start:])
                        validated = json.loads(json_str)
                        context.validated_findings.extend(validated)
                    except Exception as e:
                        logger.debug(f"Could not parse VALIDATED_FINDINGS: {e}")

        # If no structured output, use raw findings as fallback
        if not context.validated_findings:
            context.validated_findings = context.raw_findings
...
class MultiAgentOrchestrator:
    ...
    def run_full_audit(self) -> Dict[str, Any]:
        """Run the complete multi-agent pentest"""
        start_time = datetime.now()
        context = AgentContext(target_url=self.target_url)

        self.log("INFO", "=" * 60)
        self.log("INFO", f"Starting Multi-Agent Pentest on {self.target_url}")
        self.log("INFO", "=" * 60)
        ...
        try:
            ...
            # Check if recon worked (at least some iterations done with tool use)
            recon_did_anything = (
                recon_result["iterations"] > 0
                and len(recon_result.get("tools_used", {})) > 0
            )
            if not recon_did_anything:
                # Recon didn't use any tools - LLM probably failed
